...
function f(e, r, i) {
	...
	return d(e, n, l, o, null);
}
function d(t, i, l, o, s) {
	var n = {
		type: t,
		props: i,
		key: l,
...
function x(t, e, r, i, l, n, a, c, f, _) {
	var m,
		h,
		k,
		x,
	...
	for (r.__k = [], m = 0; m < e.length; m++)
		if (
			null !=
			(x = r.__k[m] =
				null == (x = e[m]) || 'boolean' == typeof x
					? null
					: 'string' == typeof x || 'number' == typeof x || 'bigint' == typeof x
					? d(null, x, null, null, x)
					: Array.isArray(x)
					? d(p, { children: x }, null, null, null)
					: x.__b > 0
					? d(x.type, x.props, x.key, x.ref ? x.ref : null, x.__v)
					: x)
		) {
			if (
				((x.__ = r),
				(x.__b = r.__b + 1),
				null === (k = b[m]) || (k && x.key == k.key && x.type === k.type))
			)
				b[m] = void 0;
			else
				for (h = 0; h < E; h++) {
					if ((k = b[h]) && x.key == k.key && x.type === k.type) {
						b[h] = void 0;
						break;
					}
					k = null;
				}
			P(t, x, (k = k || o), l, n, a, c, f, _),
				(L = x.__e),
				(h = x.ref) &&
					k.ref != h &&
					(v || (v = []),
					k.ref && v.push(k.ref, null, x),
					v.push(h, x.__c || L, x)),
				null != L
					? (null == w && (w = L),
					  'function' == typeof x.type && x.__k === k.__k
							? (x.__d = f = y(x, f, t))
							: (f = u(t, x, k, b, L, f)),
					  'function' == typeof r.type && (r.__d = f))
					: f && k.__e == f && f.parentNode != t && (f = g(k));
		}
	for (r.__e = w, m = E; m--; ) null != b[m] && O(b[m], b[m]);
	if (v) for (m = 0; m < v.length; m++) T(v[m], v[++m], v[++m]);
}
function y(t, e, r) {
	for (var i, l = t.__k, o = 0; l && o < l.length; o++)
		(i = l[o]) &&
			((i.__ = t),
			(e = 'function' == typeof i.type ? y(i, e, r) : u(r, i, i, l, i.__e, e)));
	return e;
}
...
function w(t, e, r, i, l) {
	...
	t: if ('style' === e)
		...
		(o = e !== (e = e.replace(/Capture$/, ''))),
			...
			(t.l[e + o] = r),
			r
				? i || t.addEventListener(e, o ? b : v, o)
				: t.removeEventListener(e, o ? b : v, o);
...
function P(t, r, i, l, o, s, n, c, f) {
	...
	try {
		t: if ('function' == typeof U) {
			if (
				((L = r.props),
				(w = (d = U.contextType) && l[d.__c]),
				(v = d ? (w ? w.props.value : d.__) : l),
				i.__c
				...
				null != U.getDerivedStateFromProps &&
					...
					a(g.__s, U.getDerivedStateFromProps(L, g.__s))),
				(h = g.props),
				(k = g.state),
				m)
			)
				...
				if (
					(null == U.getDerivedStateFromProps &&
						...
						g.componentWillReceiveProps(L, v),
					(!g.__e &&
						null != g.shouldComponentUpdate &&
						!1 === g.shouldComponentUpdate(L, g.__s, v)) ||
						r.__v === i.__v)
				) {
					for (
						g.props = L,
				...
				null != g.componentWillUpdate && g.componentWillUpdate(L, g.__s, v),
					null != g.componentDidUpdate &&
						g.__h.push(function () {
							g.componentDidUpdate(h, k, y);
						});
			}
			if (
				((g.context = v),
			...
			(g.state = g.__s),
				...
				m ||
					...
					(y = g.getSnapshotBeforeUpdate(h, k)),
				(O = null != d && d.type === p && null == d.key ? d.props.children : d),
				x(t, Array.isArray(O) ? O : [O], r, i, l, o, s, n, c, f),
				(g.base = r.__e),
				(r.__h = null),
...
function S(e, r, i, l, s, n, a, f) {
	...
	if (null === k) m === h || (f && e.data === h) || (e.data = h);
		...
		if (
			...
			(p = (m = i.props || o).dangerouslySetInnerHTML),
			...
			(_ || p) &&
				((_ && ((p && _.__html == p.__html) || _.__html === e.innerHTML)) ||
		...
		if (
			((function (t, e, r, i, l) {
				var o;
				for (o in r)
					'children' === o || 'key' === o || o in e || w(t, o, null, r[o], i);
				for (o in e)
			...
			for (y = n.length; y--; ) null != n[y] && c(n[y]);
		f ||
			('value' in h &&
				void 0 !== (y = h.value) &&
				(y !== e.value ||
					('progress' === k && !y) ||
					('option' === k && y !== m.value)) &&
				w(e, 'value', y, m.value, !1),
			'checked' in h &&
				void 0 !== (y = h.checked) &&
				y !== e.checked &&
				w(e, 'checked', y, m.checked, !1));
	}
	return e;
}
function T(t, r, i) {
...
function O(t, r, i) {
	...
	if (
		...
		(l = t.ref) && ((l.current && l.current !== t.__e) || T(l, null, r)),
		null != (l = t.__c))
	) {
	...
	if ((l = t.__k))
		for (o = 0; o < l.length; o++)
			l[o] && O(l[o], r, i || 'function' != typeof t.type);
...
!(function (r, i, l) {
	...
	M`<${function () {
		return M`<main><h1>Snertloop</h1><fieldset><input id="t16" type="checkbox" checked/><label for="t16">16km</label><input id="t11" type="checkbox" checked/><label for="t11">11km</label><input id="t5" type="checkbox" checked/><label for="t5">5km</label><input id="tc" type="checkbox" checked/><label for="tc">Verkeersregelaar</label><input id="t16km" type="checkbox" checked/><label for="t16km">16km labels</label></fieldset><div class="map-container"><img src="assets/map.svg" alt="map"/><${G}/></div><aside><h2><label for="legend">Legenda</label></h2><input id="legend" type="checkbox" checked/><dl><dt class="track track16">Rode lijn</dt><dd><label for="t16">Route 10 EM (16.1 km)</label></dd><dt class="track track11">Blauwe lijn</dt><dd><label for="t11">Route 11 km</label></dd><dt class="track track5">Groene lijn</dt><dd><label for="t5">Route 5 km</label></dd><dt><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" style="isolation:isolate" viewBox="0 0 10 10" width="10pt" height="10pt"><circle vector-effect="non-scaling-stroke" cx="4.999999999999999" cy="4.999999999999999" r="3" fill="#fff" stroke-width="2" stroke="rgb(0,0,0)" stroke-linejoin="miter" stroke-linecap="square" stroke-miterlimit="3"/></svg></dt><dd>Start/Finish</dd><dt class="svg svg--tc"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" style="isolation:isolate" viewBox="0 0 10 10" width="10pt" height="10pt"><defs><clipPath id="_clipPath_4we1JPgjuo6K7oOrPic3p0dH17ZzqA8v"><rect width="10" height="10"/></clipPath></defs><g clip-path="url(#_clipPath_4we1JPgjuo6K7oOrPic3p0dH17ZzqA8v)"><g><radialGradient id="_rgradient_0" fx="0.5" fy="0.5" cx="0.5" cy="0.5" r="0.5" gradientTransform="matrix(10.062,0,0,10.062,-0.031,-0.031)" gradientUnits="userSpaceOnUse"><stop offset="0%" stop-opacity="1" style="stop-color:rgb(255,255,0)"/><stop offset="99.16666666666667%" stop-opacity="1" style="stop-color:rgb(202,202,0)"/></radialGradient><mask id="_mask_vsE6NPYwaYugom64MXwAGD3uxi7MEbwI"><circle vector-effect="non-scaling-stroke" cx="4.999999999999999" cy="4.999999999999999" r="5.031249999999999" fill="white" stroke="none"/></mask><circle vector-effect="non-scaling-stroke" cx="4.999999999999999" cy="4.999999999999999" r="5.031249999999999" fill="url(#_rgradient_0)"/><circle vector-effect="non-scaling-stroke" cx="4.999999999999999" cy="4.999999999999999" r="5.031249999999999" fill="url(#_rgradient_0)" mask="url(#_mask_vsE6NPYwaYugom64MXwAGD3uxi7MEbwI)" stroke-width="2" stroke="rgb(0,0,0)" stroke-linejoin="miter" stroke-linecap="square" stroke-miterlimit="3"/><g clip-path="url(#_clipPath_QYmJmy2hQDfMMn5OLC07GiufOeX5nWZN)"><text transform="matrix(1,0,0,1,2.975,7.328)" style="font-family:sans-serif;font-weight:700;font-size:6px;font-style:normal;fill:#000000;stroke:none;">1</text></g><defs><clipPath id="_clipPath_QYmJmy2hQDfMMn5OLC07GiufOeX5nWZN"><rect x="0" y="0" width="6" height="8.171" transform="matrix(1,0,0,1,2.688,0.915)"/></clipPath></defs></g></g></svg></dt><dd><label for="tc">Verkeersregelaar</label></dd><dt class="svg svg--km"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" style="isolation:isolate" viewBox="0 0 13 13" width="13pt" height="13pt"><defs><clipPath id="_clipPath_rfsl1tYdLnWZkwH93LYvYDJOvYSE5C76"><rect width="13" height="13"/></clipPath></defs><g clip-path="url(#_clipPath_rfsl1tYdLnWZkwH93LYvYDJOvYSE5C76)"><g><rect x="2.578" y="2.578" width="7.843" height="7.843" transform="matrix(0.707,0.707,-0.707,0.707,6.5,-2.692)" fill="rgb(255,255,255)" vector-effect="non-scaling-stroke" stroke-width="1" stroke="rgb(255,25,25)" stroke-linejoin="miter" stroke-linecap="square" stroke-miterlimit="2"/><g clip-path="url(#_clipPath_R1xuVRxyDeaOZ5f61MSFm1vOrGbxtdQN)"><text transform="matrix(1,0,0,1,4.506,8.796)" style="font-family:sans-serif;font-weight:700;font-size:6px;font-style:normal;fill:#000000;stroke:none;">1</text></g><defs><clipPath id="_clipPath_R1xuVRxyDeaOZ5f61MSFm1vOrGbxtdQN"><rect x="0" y="0" width="6" height="8.171" transform="matrix(1,0,0,1,4.219,2.383)"/></clipPath></defs></g></g></svg></dt><dd><label for="t16km">Kilometerpunt (16,1 km)</label></dd></dl></aside></main>`;
	}}/>`,
	document.body
);
